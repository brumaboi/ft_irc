...
{
    server.sendResponse(fd, Ui::BLUE + serverPrefix(server) + " NOTICE " + target + ":" + Ui::RESET + message + "\r\n");
}

inline void sendError(Server& server, int fd, const std::string& target, const std::string& message)
{
    server.sendResponse(fd, Ui::BLUE + serverPrefix(server) + " NOTICE " + target + ":" + Ui::RED + message + Ui::RESET + "\r\n");
}

inline void sendWarning(Server& server, int fd, const std::string& target, const std::string& message)
{
    server.sendResponse(fd, Ui::BLUE + serverPrefix(server) + " NOTICE " + target + ":" + Ui::YELLOW + message + Ui::RESET + "\r\n");
}

inline void sendSuccess(Server& server, int fd, const std::string& target, const std::string& message)
{
    server.sendResponse(fd, Ui::BLUE + serverPrefix(server) + " NOTICE " + target + ":" + Ui::GREEN + message + Ui::RESET + "\r\n");
}
